...
const info = () => page.evaluate(() => ({
  ...
  divs: document.querySelectorAll('.absolute.inset-0.z-10').length,
  visibleHeading: document.querySelector('h1,h2')?.textContent?.trim(),
}));
...
console.log("theme:", JSON.stringify(await info()));
await page.getByText("Birthday",{exact:true}).tap(); await page.waitForTimeout(300);
await tapBtn("Continue"); await page.waitForTimeout(900);
console.log("->layout:", JSON.stringify(await info()));
await page.getByText("4 Grid",{exact:true}).tap(); await page.waitForTimeout(300);
await tapBtn("Start shooting"); await page.waitForTimeout(900);
console.log("->capture:", JSON.stringify(await info()));
// go back to layout via Back button on capture
await page.getByText("Back",{exact:true}).tap().catch(()=>{}); await page.waitForTimeout(900);
console.log("back->layout:", JSON.stringify(await info()));
await browser.close();
